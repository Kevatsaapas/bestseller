...
public class DataController {
	...
	private String[] spostit = { "haaga.helia@haaga-helia.com", "metro@polia.com", "laurea.ammatti@korkea.com",
			"business.college@helsinki.com", "turku.amk@turkuamk.com", "jamk.amk@jamkamk.com" };
	...
	public String dataa(@PathVariable Long kilpailuId, Model model) {
		...
		List<OstajaArviointi> finaaliostajaarvioinnit = ostajaArviointiRepository.findByKilpailuIdAndLohko(kilpailuId,
				finaalilohko);
		...
		if (kilpailu.getFinaali() == 0 && arviointiTotal == arvioinnit.size() && ostajaArvioinnit.size() > 0
				&& ostajaArviointiTotal == ostajaArvioinnit.size() && ostajaArvioinnit.size() > 0) {
	...
	public String finaalidataa(@PathVariable Long kilpailuId, Model model) {
		...
		List<OstajaArviointi> ostajaArvioinnit = ostajaArviointiRepository.findByKilpailuIdAndLohko(kilpailuId,
				finaalilohko);
		...
		if (kilpailu.getFinaali() == 1 && kilpailu.getValmis() == 0 && arviointiTotal == arvioinnit.size()
				&& arvioinnit.size() > 0 && ostajaArviointiTotal == ostajaArvioinnit.size()
				&& ostajaArvioinnit.size() > 0) {
	...
	public void luoDatat(Kilpailu kilpailu) {
		...
		for (int u = 1; u < 5; u++) {
			...
			for (int luku = 1; luku < 3; luku++) {
				...
				Tuomari tuomari = new Tuomari("Tuomari " + luku, etunimet[randomNum], sukunimet[randomNumm], lohkonro,
						kilpailuId, finaaliin);
