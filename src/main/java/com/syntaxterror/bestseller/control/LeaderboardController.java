...
public class LeaderboardController {
	...
	private String kilpailijanTulokset(@PathVariable String kilHash, Model model, HttpServletRequest request) {
		...
		if(kil.finaalissa==1) {
			...
			List<OstajaArviointi> finaaliOstajaArvioinnit = ostajaArviointiRepository.findByKilpailuIdAndLohko(kilp.getkilpailuId(), finaalilohko);
	...
	private String Seuraaa(@PathVariable("kilpailuId") Long kilpailuId, Model model) {
		...
		if (kilpailu.getAuki().equals(new Long(0))) {
			...
			if (kilpailu.getFinaali().equals(new Long(0))) {
				...
				double totalProsentti = (double) (arviointiLkm + ostajaArviointiLkm)
						/ (arviointiTotal + ostajaArviointiTotal) * 100;
	...
	private List<List<Kilpailija>> jaaTulokset(Long kilpailuId) {
		...
		finaali.add(eka.get(0));
		finaali.add(toka.get(0));
		finaali.add(kolmas.get(0));
		finaali.add(neljas.get(0));
